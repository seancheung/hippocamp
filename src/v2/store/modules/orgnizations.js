...
export default {
    ...
    actions: {
        list({commit}) {
            commit(types.BEGIN);
            return Promise.cast(Vue.http.get('/api/v2/orgnizations'))
                .then(res => {
        ...
        show({commit}, id) {
            commit(types.BEGIN);
            return Promise.cast(Vue.http.get(`/api/v2/orgnizations/${id}`))
                .then(res => {
        ...
        update({commit}, {id, context}) {
            commit(types.BEGIN);
            return Promise.cast(Vue.http.put(`/api/v2/orgnizations/${id}`, context))
                .then(res => {
        ...
        create({commit}, context) {
            commit(types.BEGIN);
            return Promise.cast(Vue.http.post(`/api/v2/orgnizations`, context))
                .then(res => {
        ...
        remove({commit}, id) {
            commit(types.BEGIN);
            return Promise.cast(Vue.http.delete(`/api/v2/orgnizations/${id}`))
                .then(res => {
                    commit(types.REMOVE, {});
                })
                .catch(res => {
